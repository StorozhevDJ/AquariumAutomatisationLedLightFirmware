
#include "lcd_futaba.h"


unsigned char space_cntr;
//extern unsigned char plat_def;

__flash unsigned char russian_charset[] = {
};
//==============================================================================



//==============================================================================
void LCD_SendChar(void *c)
{
char *ch=c;
LCD_SendData(russian_charset[ch[0]]);
}



//==============================================================================
void LCD_SendUChar(unsigned char x)
{
LCD_SendData(russian_charset[x]);
}


//==============================================================================
void LCD_WriteStr(void * str)
{
unsigned char *ptr=str;
while (*ptr) LCD_SendData(russian_charset[*ptr++]);
}



//==============================================================================
void LCD_WriteStrCCF(const char __flash * str)
{
do	{
	if (*str == 0) break;
	LCD_SendData(russian_charset[*str++]);
	} while (1);
}



//==============================================================================
void LCD_WriteStrUCF(unsigned char __flash * str)
{
do	{
	if (*str == 0) break;
	LCD_SendData(russian_charset[*str++]);
	} while (1);
}



//==============================================================================
void LCD_WriteStrUC(unsigned char * str)
{
do {
	if (*str == 0) break;
	LCD_SendData(russian_charset[*str++]);
	} while (1);
}



//==============================================================================
void LCD_WriteStrUCCount(unsigned char * str, unsigned char count)
{
unsigned char qq=0;
do	{
	if (*str == 0) break;
	LCD_SendData(russian_charset[*str++]);
	qq++;
	} while (qq<count);
}



//==============================================================================
unsigned char LCD_WriteStrUCF20(const char __flash * str)
{
unsigned char i=0;
do	{
	if (*str == 0) break;
	if (i++>=20) return i;
	LCD_SendData(russian_charset[*str++]);
	} while (1);
return i;
}



//==============================================================================
void LCD_WriteStrUCF80(unsigned char __flash * str)
{
unsigned char i=0;
do	{
	if (i++>=80) return;
	if (*str == 0) break;
	LCD_SendData(russian_charset[*str++]);
	} while (1);
}



//==============================================================================
void LCD_WriteStrUCFCount(unsigned char __flash * str, unsigned char count)
{
do	{
	if (*str == 0) break;
	LCD_SendData(russian_charset[*str++]);
	} while (count--);
}



//==============================================================================
void LCD_WriteByteHex(unsigned char data)
{
char temp;
temp = data >> 4;
if (temp > 0x09) LCD_SendData((temp + 0x37)); else LCD_SendData((temp + 0x30));
temp = data & 0x0F;
if (temp > 0x09) LCD_SendData((temp + 0x37)); else LCD_SendData((temp + 0x30));
}



//==============================================================================
void LCD_WriteByteDec(unsigned char data)
 {
  unsigned char a,b,c;
  a=(data % 10);
  data=data/10;
  b=(data % 10);
  c=data/10;
  if (c!=0)
   {
    LCD_SendData((c | 0x30));
    LCD_SendData((b | 0x30));
    LCD_SendData((a | 0x30));
    return;
   }
  if (b!=0)
   {
    LCD_SendData((b | 0x30));
    LCD_SendData((a | 0x30));
    return;
   }
  LCD_SendData((a | 0x30));
}



//==================================================================================
void LCD_WriteByteSDec(signed char data)
 {
  if (data < 0) {LCD_SendData('-');data=-data;}
  LCD_WriteByteDec(data);
 }


//==================================================================================
void LCD_WriteIntDec(unsigned int data)
 {
  unsigned char a,b,c,d,e;
  a=(data % 10);
  data=data/10;
  b=(data % 10);
  data=data/10;
  c=(data % 10);
  data=data/10;
  d=(data % 10);
  e=data/10;
  if (e!=0)
   {
    LCD_SendData((e | 0x30));
    LCD_SendData((d | 0x30));
    LCD_SendData((c | 0x30));
    LCD_SendData((b | 0x30));
    LCD_SendData((a | 0x30));
    return;
   }
  if (d!=0)
   {
    LCD_SendData((d | 0x30));
    LCD_SendData((c | 0x30));
    LCD_SendData((b | 0x30));
    LCD_SendData((a | 0x30));
    return;
   }
  if (c!=0)
   {
    LCD_SendData((c | 0x30));
    LCD_SendData((b | 0x30));
    LCD_SendData((a | 0x30));
    return;
   }
  if (b!=0)
   {
    LCD_SendData((b | 0x30));
    LCD_SendData((a | 0x30));
    return;
   }
  LCD_SendData((a | 0x30));
 }


//==================================================================================
void LCD_WriteSIntDec(signed int data)
 {
  if (data < 0) {LCD_SendData('-');data=-data;}
  LCD_WriteIntDec(data);
 }



//**********************************************************************************
void LCD_WriteLongDec(unsigned long data)
 {
  unsigned char a[10];
  unsigned char tmp,tmp1;
  if (data<10) {LCD_SendData(data+0x30);return;}
  for (tmp=0;tmp<10;tmp++)
   {
    a[tmp]=(data % 10);
    data=data/10;
   }
  for (tmp=9;tmp>0;tmp--)
   {
    if (a[tmp]!=0)
     {
      for (tmp1=tmp;tmp1>0;tmp1--)
       LCD_SendData((a[tmp1]|0x30));
      LCD_SendData((a[0]|0x30));
      return;
     }
   }
 }



//==================================================================================
void LCD_WriteLongSDec(signed long data)
 {
  if (data < 0) {LCD_SendData('-');data=-data;}
  LCD_WriteLongDec(data);
 }



//**********************************************************************************
void LCD_WriteByteDecDot(unsigned char data, unsigned char dot)
 {
  unsigned char x,y;
  unsigned char digit[mantiss+1];
  if (dot==0)
   {// запятой нету, выводим как обычно
    LCD_WriteByteDec(data);
    return;
   }
  x=mantiss;
  while (1)
   {
    digit[x--]=((data % 10)| 0x30);
    data=data/10;
    if (dot==(mantiss-x)) {digit[x--]='.';}
    if ((data==0x00)&&(dot<(mantiss-x)))
     {
      if (digit[x+1]=='.') digit[x]='0'; else x++;
      break;
     }
   }
  for (y=x;y<(mantiss+1);y++)
   LCD_SendData(digit[y]);
 }



//**********************************************************************************
void LCD_WriteByteSDecDot(signed char data, unsigned char dot)
 {
  unsigned char x,y=0;
  unsigned char digit[mantiss+1];
  if (dot==0)
   {// запятой нету, выводим как обычно
    LCD_WriteByteSDec(data);
    return;
   }
  if (data<0) {data=-data;y=1;}
  x=mantiss;
  while (1)
   {
    digit[x--]=((data % 10)| 0x30);
    data=data/10;
    if (dot==(mantiss-x)) {digit[x--]='.';}
    if ((data==0x00)&&(dot<(mantiss-x)))
     {
      if (digit[x+1]=='.') digit[x]='0'; else x++;
      break;
     }
   }
  if (y==1) LCD_SendData('-');
  for (y=x;y<(mantiss+1);y++)
   LCD_SendData(digit[y]);
 }



//**********************************************************************************
void LCD_WriteIntDecDot(unsigned int data, unsigned char dot)
 {
  unsigned char x,y;
  unsigned char digit[mantiss+1];
  if (dot==0)
   {// запятой нету, выводим как обычно
    LCD_WriteIntDec(data);
    return;
   }
  x=mantiss;
  while (1)
   {
    digit[x--]=((data % 10)| 0x30);
    data=data/10;
    if (dot==(mantiss-x)) {digit[x--]='.';}
    if ((data==0x00)&&(dot<(mantiss-x)))
     {
      if (digit[x+1]=='.') digit[x]='0'; else x++;
      break;
     }
   }
  for (y=x;y<(mantiss+1);y++)
   LCD_SendData(digit[y]);
 }



//**********************************************************************************
void LCD_WriteIntSDecDot(signed int data, unsigned char dot)
 {
  unsigned char x,y=0;
  unsigned char digit[mantiss+1];

  if (dot==0)
   {// запятой нету, выводим как обычно
    LCD_WriteSIntDec(data);
    return;
   }
  if (data<0) {data=-data;y=1;}
  x=mantiss;
  while (1)
   {
    digit[x--]=((data % 10)| 0x30);
    data=data/10;
    if (dot==(mantiss-x)) {digit[x--]='.';}
    if ((data==0x00)&&(dot<(mantiss-x)))
     {
      if (digit[x+1]=='.') digit[x]='0'; else x++;
      break;
     }
   }
  if (y==1) LCD_SendData('-');
  for (y=x;y<(mantiss+1);y++)
   LCD_SendData(digit[y]);

 }



//**********************************************************************************
void LCD_WriteByteBin(char data)
 {
  unsigned char i;
  unsigned char temp;
  temp = data;
  for (i = 8; i > 0; i--)
   {
    if ((temp & 0x80) == 0x80) LCD_SendData((0x31)); else LCD_SendData((0x30));
    temp <<= 1;
   }
 }



//**********************************************************************************
void LCD_WriteLongDecDot(unsigned long data, unsigned char dot)
 {
  unsigned char x,y;
  unsigned char digit[mantiss+1];
  if (dot==0)
   {// запятой нету, выводим как обычно
    LCD_WriteLongDec(data);
    return;
   }
  x=mantiss;
  while (1)
   {
    digit[x--]=((data % 10)| 0x30);
    data=data/10;
    if (dot==(mantiss-x)) {digit[x--]='.';}
    if ((data==0x00)&&(dot<(mantiss-x)))
     {
      if (digit[x+1]=='.') digit[x]='0'; else x++;
      break;
     }
   }
  for (y=x;y<(mantiss+1);y++)
   LCD_SendData(digit[y]);
 }



//**********************************************************************************
void LCD_WriteLongSDecDot(signed long data, unsigned char dot)
 {
  unsigned char x,y=0;
  unsigned char digit[mantiss+1];
  if (dot==0)
   {// запятой нету, выводим как обычно
    LCD_WriteLongSDec(data);
    return;
   }
  if (data<0) {data=-data;y=1;}
  x=mantiss;
  while (1)
   {
    digit[x--]=((data % 10)| 0x30);
    data=data/10;
    if (dot==(mantiss-x)) {digit[x--]='.';}
    if ((data==0x00)&&(dot<(mantiss-x)))
     {
      if (digit[x+1]=='.') digit[x]='0'; else x++;
      break;
     }
   }
  if (y==1) LCD_SendData('-');
  for (y=x;y<(mantiss+1);y++)
   LCD_SendData(digit[y]);
 }



//==================================================================================
void LCD_WriteIntDecFn(unsigned int data)
 {
  unsigned char a,b,c,d;
  a=(data % 10);
  data=data/10;
  b=(data % 10);
  data=data/10;
  c=(data % 10);
  data=data/10;
  d=(data % 10);
  LCD_SendData((d | 0x30));
  LCD_SendData((c | 0x30));
  LCD_SendData((b | 0x30));
  LCD_SendData((a | 0x30));
 }



void LCD_WriteIntDecExtFn(unsigned int data)
 {
  unsigned char a,b,c,d,e;
  a=(data % 10);
  data=data/10;
  b=(data % 10);
  data=data/10;
  c=(data % 10);
  data=data/10;
  d=(data % 10);
  data=data/10;
  e=(data % 10);
  LCD_SendData((e | 0x30));
  LCD_SendData((d | 0x30));
  LCD_SendData((c | 0x30));
  LCD_SendData((b | 0x30));
  LCD_SendData((a | 0x30));
 }



//==================================================================================
void LCD_WriteHexArr(unsigned char * data, unsigned char kol)
 {
  unsigned char temp,i;
  for (i=0;i<kol;i++)
   {
    temp = data[i] >> 4;
    if (temp > 0x09) LCD_SendData((temp + 0x37)); else LCD_SendData((temp + 0x30));
    temp = data[i] & 0x0F;
    if (temp > 0x09) LCD_SendData((temp + 0x37)); else LCD_SendData((temp + 0x30));
   }
 }

